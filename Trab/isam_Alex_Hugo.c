...
void isam_insere(T_ISAM *isam,void *dado){
    ...
    int tam_arq = func_tam_arq(arq_dados);
    TNo_ISAM * no_isam = isam_criar_no(isam,INTERNO);
    func_exportar_arq_texto(arq_dados, "arquivo_dados_novos.txt");
    isam_ler_no_pos(isam,no_isam, isam->raiz);
    TIndiceReg PP = isam->raiz;
    ...
    for (int u =0; u<isam->t; u++){
        if (no_isam->filhos[u] == -1){
            no_isam->filhos[u] = tam_arq;
            no_isam->chaves[u] = tam_arq;
            break;
        }
    }

    isam_salvar_no_pos(isam,no_isam, PP);
