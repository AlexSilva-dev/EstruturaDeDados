...
TGrafo *grafo_retira_vertice(TGrafo *g, int v){
    ...
    while( atual){
        ...
        rViz = atual->prim_vizinho;
        while (rViz){
            rvAux = rViz;
            if(rViz->id_vizinho == v){
                rvAux->prox = rViz->prox;
                free(rViz);
                break;
            }
            rViz = rViz->prox;
    ...
    TGrafo *p = grafo_busca_vertice(g, v);
    TVizinho *vAux1 = p->prim_vizinho;
    while (p->prim_vizinho)
    {
        p->prim_vizinho = vAux1->prox;
        printf("\n%d\n", vAux1->id_vizinho);
        free(vAux1);
        vAux1 = p->prim_vizinho;
    }
